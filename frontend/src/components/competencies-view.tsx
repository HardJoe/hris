  ...
  return (
    <>
      ...
      <section className="card">
        ...
        {!result ? <div className="loading-state">Loading competencies…</div> : result.items.length === 0 ? <div className="empty-state">No competencies found.</div> : <div className="table-wrap responsive-table-wrap"><table className="table responsive-table competency-table"><thead><tr><th className="table-heading">Competency</th><th className="table-heading">Created</th><th className="table-heading">Updated</th><th className="table-heading"><span className="sr-only">Actions</span></th></tr></thead><tbody>{result.items.map((item) => <tr className="table-row" key={item.id}><td className="table-cell" data-label="Competency"><strong>{item.name}</strong></td><td className="table-cell" data-label="Created">{formatTimestamp(item.createdAt)}</td><td className="table-cell" data-label="Updated">{formatTimestamp(item.updatedAt)}</td><td className="table-cell responsive-actions-cell"><div className="actions"><button className="icon-button" type="button" onClick={() => setEditing(item)} aria-label={`Edit ${item.name}`}><Pencil size={17} strokeWidth={2} aria-hidden="true" /></button><button className="icon-button" type="button" onClick={() => setDeleting(item)} aria-label={`Delete ${item.name}`}><Trash2 size={17} strokeWidth={2} aria-hidden="true" /></button></div></td></tr>)}</tbody></table></div>}
